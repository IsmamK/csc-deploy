...
const handleIconUpload = async (serviceIndex, featureIndex, file) => {
  try {
  ...
  const handleHeroImageUpload = async (serviceIndex, file) => {
    try {
      const base64Image = await convertToBase64(file);
      updateService(serviceIndex, 'heroImage', base64Image);
    } catch (error) {
      ...
      {servicesData.map((service, serviceIndex) => (
        <div key={serviceIndex} className="bg-white p-6 rounded-lg shadow-md hover:shadow-xl transition-all duration-300">
          ...
          <div>
                ...
                />
                {service.heroImage && (
                  <img src={service.heroImage} alt="Feature Icon" className="h-80 w-96 mb-2" />
                )}
              </div>
