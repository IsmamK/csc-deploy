...
const B2BModal = ({ isOpen, onClose }) => {
  ...
  useEffect(() => {
    const fetchProjects = async () => {
      const response = await fetch(`${API_URL}/b2b/`); // Fetch projects data
  ...
  const handleSaveAll = async () => {
    const response = await fetch(`${API_URL}/b2b/`, {
      method: 'PATCH',
      headers: {
  ...
  return (
    <dialog id="b2b-modal" className={`modal ${isOpen ? 'modal-open' : ''}`}>
      <div className="modal-box w-11/12 max-w-5xl relative">
        <button onClick={onClose} className="btn btn-sm btn-circle absolute right-2 top-2">
          ✕
        </button>
        <h3 className="font-bold text-lg mb-4">Business to Business</h3>
