...
const ServicesModal = ({ isOpen, onClose }) => {
  ...
  const handleIconChange = (e) => {
    ...
    if (file) {
      ...
      reader.onloadend = () => {
        setNewService({ ...newService, icon: reader.result });
      };
      reader.readAsDataURL(file); // Convert the image file to base64
    }
  };

  const handleIconEditChange = (index, e) => {
    const file = e.target.files[0];
    if (file) {
      const reader = new FileReader();
      reader.onloadend = () => {
        const updatedServices = [...servicesData.services];
        updatedServices[index].icon = reader.result;
        setServicesData({ ...servicesData, services: updatedServices });
      };
      reader.readAsDataURL(file); // Convert the image file to base64
    }
            ...
            {servicesData.services.map((service, index) => (
              <tr key={index}>
                ...
                <td>
                  <input
                    type="file"
                    onChange={(e) => handleIconEditChange(index, e)}
                    className="file-input file-input-bordered w-full mb-2"
                  />
                  {service.icon && (
                    <img
