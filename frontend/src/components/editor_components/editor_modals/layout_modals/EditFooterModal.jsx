...
const EditFooterModal = ({ isOpen, onClose, footerData, setFooterData }) => {
  ...
  const [newFooterData, setNewFooterData] = useState({
    ...
    logo: footerData?.logo || null, // Add logo field
  });
  ...
  const handleLogoUpload = async (event) => {
    ...
    if (file) {
      try {
        ...
        setNewFooterData({ ...newFooterData, logo: base64Logo });
      } catch (error) {
  ...
  {/* Logo Upload */}
  <div className="mb-6">
          <label htmlFor="logo" className="block text-sm font-medium">Footer Logo</label>
          <input
            type="file"
            id="logo"
            className="border p-2 w-full rounded-md"
