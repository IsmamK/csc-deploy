...
const PartnerCarouselModal = ({ isOpen, onClose }) => {
  ...
  useEffect(() => {
    ...
    const fetchImages = async () => {
      try {
        const response = await fetch(`${apiUrl}/partner-carousel`);
        if (!response.ok) {
  ...
  return (
    <dialog className={`modal ${isOpen ? 'modal-open' : ''}`}>
      <div className="modal-box w-11/12 max-w-2xl relative">
        ...
        <h3 className="font-bold text-lg mb-4">Edit Partner Carousel Images</h3>
...
export default PartnerCarouselModal;
